...
Die reellen Zahlen bilden den bis auf Isomrphie eindeutig bestimmten \begr{vollständig angeordneten Körper}.
...
Die Menge $\mathbb{C} = \{x + i\cdot y\,|\,x, y \in \mathbb{R}\}$ heißt \begr{Menge der komplexen Zahlen}.

Die Zahl $i = 0 + 1\cdot i$ ist definiert als $i^2 = -1$ und heißt \begr{imaginäre Einheit}. Für eine Zahl $z = x + i\cdot y$ heißt $x$ \begr{Realteil von z} ($\text{Re}(z) := x$), $y$ heißt \begr{Imaginärteil von z} ($\text{Im}(z) := y$).
