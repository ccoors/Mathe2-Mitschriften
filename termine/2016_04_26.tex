...
\begin{align*}
    ...
    \end{pmatrix}
\end{align*}
kurz $A = (a_{ij})$ heißt \begr{$m \times n$ Matrix}. $(m, n)$ heißt \begr{Dimension} der Matrix, $a_{ij}$ heißen \begr{Einträge}/\begr{Koeffizienten}. Ist $A$ eine $n \times n$ Matrix (quadratische Matrix), so heißen die Koeffizienten $a_{ii}$ \begr{Diagonalelemente} von $A$.
...
\mdf{Definition}
Zu einer $m \times n$ Matrix $A = (a_{ij})$ heißt die $n \times m$ Matrix $A^T = (a_{ji})$ \begr{transponierte Matrix} zu $A$.
...
\begin{align*}
    A &= A^T
\end{align*}
so heißt $A$ \begr{symmetrische Matrix}.
...
\begin{align*}
    ...
    \end{pmatrix}
\end{align*}
heißt \begr{$n \times n$ Einheitsmatrix}. Ist $A \in \mathbb{R}^{m \times n}$, so gilt $\mathbbm{1}_m A = A \mathbbm{1}_n = A$.
